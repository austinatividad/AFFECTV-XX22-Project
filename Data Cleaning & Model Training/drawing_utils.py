...
def draw_landmarks(
    ...
    Args:
        ...
        ImportantLandmarks: A list of indices representing the important landmarks.
    ...
    if is_drawing_landmarks and landmark_drawing_spec:
        for idx, landmark_px in idx_to_coordinates.items():
            ...
            if idx in ImportantLandmarks:
                print("Important Landmark found!: ", idx)
                # If the landmark index is in the list of ImportantLandmarks, color it yellow
                cv2.circle(image, landmark_px, max(drawing_spec.circle_radius + 3,
                                       int(drawing_spec.circle_radius * 1.2)), BLUE_COLOR,
                       drawing_spec.thickness)

                ImportantLandmarks_index = ImportantLandmarks.index(idx)
                cv2.putText(image, f"{idx}", landmark_px, cv2.FONT_HERSHEY_SIMPLEX, 1, BLUE_COLOR, 5)
                cv2.putText(image, f"{idx}", landmark_px, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 3)
